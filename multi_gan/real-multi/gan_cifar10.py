...
DATA_DIR = '/data1/liangjiadong/datasets/data_cifar10/cifar-10-batches-py'
if len(DATA_DIR) == 0:
...
class Generator(nn.Module):
    def __init__(self):
        ...
        shiba = nn.Linear(128, 64 * DIM).cuda()
    ...
    def forward(self, input):
        output = self.shiba(input)
        # print(output.size())
        output = output.view(-1, 64 * DIM, 1, 1)
...
for iteration in range(ITERS):
    ...
    for i in range(CRITIC_ITERS):
        ...
        noisev = noisev.view(-1, 1, 1, 128)
        fake = autograd.Variable(netG(noisev).data)
        inputv = fake
        D_fake = netD(inputv)
        D_fake = D_fake.mean()
        D_fake.backward(one.mean())
