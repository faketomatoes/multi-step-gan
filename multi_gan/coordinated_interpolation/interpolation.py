...
for epoch in range(nepochs):
    for i, data in enumerate(dataloader, 0):
        ...
        label = torch.full((batch_size,), 0, device=device, dtype=torch.int64)
        output = netD(real) # unnormalized
        errD = criterion_MC(output, label.detach())
        ...
        for k in range(np):
            ...
            latent = torch.mul(latent_real, real_mask.detach()) + torch.mul(noise, noise_mask.detach())
            fake_buffer.append(netG(latent))
            label = torch.full((batch_size,), k + 1, device=device, dtype=torch.int64)
            output = netD(fake_buffer[k].detach())
            errD += criterion_MC(output, label.detach())
            output = netD(fake_buffer[k])
            errG -= criterion_MC(output, label.detach())
        errG += criterion_reconstruct(real, netG(latent_real))
        ...
        for j in range(10):
            ...
            for l in range(np):
                noise_mask = torch.zeros((batch_size, nz, 1, 1), device=device)
                real_mask = torch.ones((batch_size, nz, 1, 1), device=device)
                index = torch.tensor(range(l * width, (l + 1) * width), dtype=torch.int64, device=device)
                noise_mask = noise_mask.index_fill_(1, index, 1)
                real_mask = real_mask.index_fill_(1, index, 0)
                latent = torch.mul(netE(real.detach()), real_mask.detach()) + torch.mul(noise, noise_mask.detach())
                fake_buffer.append(netG(latent))
                label = torch.full((batch_size,), l + 1, device=device, dtype=torch.int64)
                output = netD(fake_buffer[l])
                GAN_loss += criterion_MC(output, label.detach())
        ...
        if i % 100 == 0:
            print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f Reconstruct_err: %.4f'
            % (epoch, nepochs, i, len(dataloader), errD.item(), errG.item(), err_reconstruct))
