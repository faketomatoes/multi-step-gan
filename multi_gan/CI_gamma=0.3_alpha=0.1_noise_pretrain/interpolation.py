...
alpha = 0.1 # coefficient for GAN_loss tern when training netE
gamma = 0.3 # coefficient for the mutual information
...
for epoch in range(nepochs):
    for i, data in enumerate(dataloader, 0):
        ...
        output = netD(real + itfr_sigma * pixd_noise)[0] # unnormalized
        errD = criterion(output, label_real)
        label_fake = torch.full((batch_size,), 1, device=device, dtype=torch.int64)
        output = netD(netG(noise) + itfr_sigma * pixg_noise)[0]
        errD += criterion(output, label_fake)
        fake_log_likelihood = criterion(1 - output, label_fake)
        errG = fake_log_likelihood + 0.0
        ...
        if i % 100 == 0:
            print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f fake_log_likelihood:%.4f CE_regularizer: %.4f Reconstruct_err: %.4f'
            % (epoch, nepochs, i, len(dataloader), errD.item(), errG.item(), fake_log_likelihood.item(), 0 - CE_regularizer.item(), err_reconstruct))
