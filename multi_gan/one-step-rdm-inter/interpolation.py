...
for epoch in range(nepochs):
    ...
    print("itpl_vl: %d, sigma: %f" % (itpl_vl, sigma))
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        lmbd = sigma * torch.randn([1], device=device) + itpl_vl/nepochs
        lmbd = np.clip(lmbd.cpu(), 0, 1)
        lmbd = lmbd.to(device)
        # print('lambda %f' % lmbd)
        netE.eval()
        latent_var = netE(real_cpu)
        latent_var = latent_var.detach()
        noise = lmbd * latent_var + (1 - lmbd) * noise
        fake = netG(noise)
